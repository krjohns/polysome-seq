...
input_dir = fs::path(wd, "../input/data/2_formatted_data/star_solo")
# For saving plots  
output_dir = fs::path(wd, "../output/2025_01_07_CtuKO_polyseq")
...
```{r, warnings =F}
#counts
counts = read_csv(fs::path(input_dir, "20250107_CtuKO_polyseq_counts.csv")) 
#rename gene column
...
#meta        
metadata = read_csv(fs::path(input_dir, "20250107_CtuKO_polyseq_metadata.csv")) %>%
           #drop unused first col
           dplyr::select(-1) %>%
           mutate(Target_Guide = str_c(Target,"_", Guide),
                  Read_sum = ifelse(colSums(counts) < 1e6, "<1e6", 
                                     ifelse(colSums(counts) > 3e6, ">3e6", "1-3e6"))) %>%
           filter(Replicate != "R2repeat") %>% #filter out repeat replicate
           filter(Read_sum != "<1e6") #filter out low reads
           #filter(!(`Pool - Sample_Num` %in% c("Polysome - 7", "Input - 2",
                       #                        "Input - 11", "Polysome - 6")))  # filter out samples that cluster poorly
...
cpm = cpm(dge_norm_fltd) #dropped to ~8,300 genes
...
#          fs::path(output_dir, "cpm_tables/low_reads_filtered/20250107_CtuKO-polyseq_cpmTable_filt_10in2.csv"))
#write.csv(as.matrix(cpm_all), 
  #        fs::path(output_dir, "cpm_tables/low_reads_filtered/20250107_CtuKO-polyseq_cpmTable_all.csv"))
...
p3
# save plots
save_plot(fs::path(output_dir, "QC/PCA/PCA_lowreads_dropped.pdf"),p,
          dpi=90, base_aspect_ratio = 1.5)
#save_plot(fs::path(output_dir, "QC/PCA/PCA_by_pool.pdf"),p2,dpi=90)
#save_plot(fs::path(output_dir, "QC/PCA/PCA_by_replicate.pdf"),p3,dpi=90,  base_aspect_ratio = 1.5)
```
...
p<-ggplot(plot_me_filt, aes(x = Target_Guide, y = avg_cpm, fill = Target, alpha = Guide)) +
  scale_fill_manual(values = c("grey90", "#D278AE", "#9E7FB8", "grey30")) +
...
p
save_plot(fs::path(output_dir, "single_gene_plots/DE_hits/chemokines.pdf"), p , base_aspect_ratio = 1.6)
```
...
write.csv(as.matrix(DE_log2FC.all), fs::path(output_dir, "DE/low_reads_filtered/contrasts_aggregated_by_target/20250107_LFC_matrix_DE_TE_FDR0.1.csv"))
```
