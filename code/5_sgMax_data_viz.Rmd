                         ...
                         Cluster = ifelse(old_cluster %in% c(18:20), 1,
                                    ifelse(old_cluster %in% c(12,14:17), 2,
                                      ifelse(old_cluster %in% c(9:11), 3,
                                       ifelse(old_cluster %in% c(6:8,13), 4,
                                          ifelse(old_cluster %in% c(1:2), 5,
                                                   ifelse(old_cluster %in% c(5), 6, 7))))))) %>%
                  arrange(Cluster)
...
write.csv(as.matrix(cluster_df_edit), 
          fs::path(output_dir,
              "DE_analysis/different_thresholds/TE-fdr0.2_DE-fdr0.1_LFC1/complex_heatmap/LFC_heatmap_7_clusters_cluster_info.csv"))

#re-order H_LFC and re-draw heatmap (please make a function for this soon kate!!!!)----
H_LFC_reorder = H_LFC[cluster_df_edit$gene,]

left_annot = cluster_df_edit %>% dplyr::select(Cluster)
...
p2<-Heatmap(H_LFC_reorder,
        ...
        row_gap = unit(c(0,0,0,0,3,0,0), "mm"),
        border = T,
        #border_gp = gpar(col = "black", lwd = 2), 
        col = colorRamp2(c(-2,-0.1, 0, 0.1, 2), c("blue","white","white", "white", "red"))
        )
...
do_plot("DE_analysis/different_thresholds/TE-fdr0.2_DE-fdr0.1_LFC1/complex_heatmap/LFC_heatmap_7_clusters-1.pdf",
        8,6,p2)
```
