...
DE_log2FC.all = log2FC.all[(log2FC.all.binary$TE != 0) + (log2FC.all.binary$sgMax_Polysome != 0) > 0,] %>%
          data.frame()
rownames(DE_log2FC.all) = rownames(log2FC.all)[(log2FC.all.binary$TE != 0) + (log2FC.all.binary$sgMax_Polysome != 0) > 0]

#filter DE matrix based on LFC thresh
LFC_thresh = 0
DE_LFC_thresh = DE_log2FC.all %>%
          filter_at(vars(TE), all_vars(abs(.) > LFC_thresh)) %>%
          dplyr::select(ends_with("Total"), ends_with("Polysome"), "TE") %>%
          rownames_to_column("gene") %>%
          mutate(fdr0.1 = gene %in% rownames(fdr0.1_table),.before = 1) 


#fdr0.1_table = DE_LFC_thresh

#write csvs of DEGs and all log2FC
write.csv(as.matrix(DE_LFC_thresh), fs::path(output_dir, "DE_analysis/FDR0.2/20240220_FDR0.2_LFC0_DE-genes-lfcTable_annotated.csv"))
