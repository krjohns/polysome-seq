...
cluster_df_edit = cluster_df %>%
                  mutate(old_cluster = Cluster, 
                         Cluster = ifelse(old_cluster %in% c(19), 1,
                                    ifelse(old_cluster %in% c(12,14:17), 2,
                                      ifelse(old_cluster %in% c(9:11), 3,
                                       ifelse(old_cluster %in% c(7,8,13), 4,
                                          ifelse(old_cluster %in% c(6,2), 5,
                                             ifelse(old_cluster %in% c(18,20), 6, 
                                                ifelse(old_cluster %in% c(5), 7, 
                                                  ifelse(old_cluster %in% c(3,4), 8, 
                                                          9))))))))) %>%
                  arrange(Cluster)
...
write.csv(as.matrix(cluster_df_edit), 
          fs::path(output_dir,
              "DE_analysis/different_thresholds/TE-fdr0.2_DE-fdr0.1_LFC1/complex_heatmap/LFC_heatmap_9_clusters_cluster_info.csv"))
...
write.csv(as.matrix(morpheus_heatmap), 
          fs::path(output_dir,
              "DE_analysis/different_thresholds/TE-fdr0.2_DE-fdr0.1_LFC1/complex_heatmap/LFC_heatmap_9_clusters_for_morpheus_update.csv"))   
...
pLFC<-Heatmap(H_LFC_reorder[,c("sgMax_Total", "sgMax_Polysome")],
        ...
        row_gap = unit(c(0,0,0,0,3,0,0,0), "mm"),
        border = T,
        #border_gp = gpar(col = "black", lwd = 2), 
        col = colorRamp2(c(-2,-0.1, 0, 0.1, 2), c("blue","white","white", "white", "red")),
        width = unit(10, "cm")
...
pTE<-Heatmap(H_LFC_reorder[,c("TE")],
        ...
        cluster_rows = F,
        cluster_columns = F,
        right_annotation = rowAnnotation(Cluster = left_annot$Cluster,
                                            col = list(Cluster = 
                                                         c(`1` = "#ED6868",
                                                           `2` = "#F4908E",
                                                           `3` = "#EED290",
...
do_plot("DE_analysis/different_thresholds/TE-fdr0.2_DE-fdr0.1_LFC1/complex_heatmap/LFC_heatmap_9_clusters-2_separateTE_coloring.pdf",
        8,6,p)
