...
counts per sample
```{r}
ggplot(counts_summary, aes(str_c(pool, '_', replicate), 
                   counts, fill=pool)) + 
    geom_col() + 
    facet_grid(treatment ~ .) + 
    #scale_y_log10() +
...
filt_cormelt = cormelt %>%
    ...
    # we're only interested in correlations less than .9
    filter(cor < .9)
...
# DESIGN MATRIX
  ...
  ##CONTRAST MATRIX
  #make more generalized later
  contrasts_list = c("LSG2h_Input - Control_Input",
                     "LSG2h_Pool1 - Control_Pool1",
                     "LSG2h_Pool2 - Control_Pool2",
                     "LSG2h_Pool3 - Control_Pool3",
                     "LSG6h_Input - Control_Input",
                     "LSG6h_Pool1 - Control_Pool1",
                     "LSG6h_Pool2 - Control_Pool2",
                     "LSG6h_Pool3 - Control_Pool3")
...
write.csv(as.matrix(DE_LFC_thresh), path(output_dir, "DE_analysis/20231101_FDR0.1_LFC1_DE-genes-lfcTable.csv"))
