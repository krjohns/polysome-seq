...
prot_input_dir = path(wd, "../../Interaction_scoring")
```
...
#DE_LFC_thresh[rownames(DE_LFC_thresh) %in% rownames(log2FC.all.binary %>% data.frame() %>% filter(TE_LSG6h != 0))],
H_per6 = H[rownames(H) %in% rownames(H_per), !(grepl("2h", colnames(H)))]
...
#reorder data to match
H_LFC6 = H_LFC6[rownames(H_per_scale_reorder),]
H_LFC_scale = H_LFC %>%
              t() %>%
              scale() %>%
              t()
...
#reorder data to match
H_TE6 = H_TE6[rownames(H_per_scale_reorder), !grepl("2h", colnames(H_TE6))]
...
cluster_df_edit_6h = cluster_df_6h %>%
                  mutate(old_cluster = Cluster, 
                         Cluster = ifelse(old_cluster %in% c(16,18:20), 1,
...
#plotting LFC and TE only ---- (need to have same # rows and be ordered the same)
temp2 <- p_LFC + p_TE + p_LFC6 + p_TE6
#save plot
#she's beautiful i love her lets save it!
...
H_TE6 = DE_LFC_thresh %>%
        ...
        as.matrix()
#reorder data to match
H_TE6 = H_TE6[rownames(H_per), !grepl("2h", colnames(H_TE6))]



#pool----

#pool LFC and TE heatmaps then replot (assume order is constant)
sum(rownames(H_LFC) == rownames(H_LFC6)) #check -- all good
sum(rownames(H_TE) == rownames(H_TE6)) # also good
H_LFC_pooled <- cbind(H_LFC, H_LFC6)
H_TE_pooled <- cbind(H_TE, H_TE6)

#annotations
LFC_pooled_annot <- rbind(topAnnot_LFC, topAnnot_LFC6)
TE_pooled_annot <- rbind(topAnnot_TE, topAnnot_TE6)

#heatmaps
p_LFC_pooled <- Heatmap(H_LFC_pooled,
        column_title = "LFC",
        column_title_gp = gpar(fontsize=12),
        row_title_gp = gpar(fontsize=7),
        name = "log2(treated/control)",
        ...
        #row_order = gene_order,
        cluster_columns = F,
        top_annotation = HeatmapAnnotation(df = LFC_pooled_annot,
                         col = list(Treatment = c(Control = "grey",
                                                   LSG2h = "#F89F7C",
                                                  LSG6h = "#F68064"),
                                   `Polysome Fraction` = c("Light" = "#68C3A5", 
                          ...
                          annotation_label = c("",""),
                          annotation_name_gp = gpar(fontsize = 8)),
        column_split = LFC_pooled_annot$Treatment,
        column_gap = unit(0, "mm"),
        row_km = 20,
        #row_split = leftAnnot6$Cluster,
        row_gap = unit(0, "mm"),
...
p_TE_pooled <- Heatmap(H_TE_pooled,
        column_title = "TE",
        column_title_gp = gpar(fontsize=12),
        row_title_gp = gpar(fontsize = 7),
        name = "log2(((heavy)(medium))/light)",
        ...
        #row_order = gene_order,
        cluster_columns = F,
        top_annotation = HeatmapAnnotation(df = TE_pooled_annot,
                         col = list(Treatment = c(Control = "grey",
                                                   LSG2h = "#F89F7C",
                                                  LSG6h = "#F68064"),
                                   Metric = c("\U0394mRNA" ="#D278AE",
                          ...
                          annotation_label = c("",""),
                          annotation_name_gp = gpar(fontsize = 8)),
        column_split = TE_pooled_annot$Treatment,
        column_gap = unit(0, "mm"),
        row_km = 20,
        #row_split = leftAnnot_pool$Cluster,
        row_gap = unit(0, "mm"),
        border = T,
        col = colorRamp2(c(-3,-1.5,-0.5 ,0,0.5, 1.5,3), c("darkblue", "#8C9FCA",
                                                            "white", "white", "white",
...
LFC_pooled_drawn <- draw(p_TE_pooled + p_LFC_pooled)

#get cluster info and reorder
LFC_pooled_drawn
rd <- row_order(LFC_pooled_drawn) #get row order & save

#save plot for now
do_plot("TE_analysis/FDR0.01/FDR0.01_LFC1/ComplexHeatmap/20231113_LFC_TE_2-6h_20Clusters.pdf",8,6,LFC_pooled_drawn)

#get cluster gene lists
cluster_list_pooled = list() #initialize empty list
for(i in 1:20){
  cluster_list_pooled[[i]] = data.frame(Cluster = i, 
                             gene = c(rownames(H_TE_pooled)[ rd[[i]]]))
}
cluster_df_pooled = do.call(bind_rows, cluster_list_pooled)

#edit/condense clusters by eye
cluster_df_edit_pooled = cluster_df_pooled %>%
                         mutate(old_cluster = Cluster, 
                         Cluster = ifelse(old_cluster %in% c(15:20), 1,
                                    ifelse(old_cluster %in% c(13,14), 2,
                                    ifelse(old_cluster %in% c(6:9), 3,
                                    ifelse(old_cluster %in% c(10:12), 4,
                                    ifelse(old_cluster %in% c(5), 5, 
                                    ifelse(old_cluster %in% c(2,3), 6, 
                                    ifelse(old_cluster %in% c(1,4), 7, 7)))))))) %>%
                         arrange(Cluster)

rownames(cluster_df_edit_pooled) = cluster_df_edit_pooled$gene

write.csv(as.matrix(cluster_df_edit_pooled), path(output_dir, "TE_analysis/FDR0.01/FDR0.01_LFC1/ComplexHeatmap/20231113_pooledLFC_TE_compiledHeatmap_7-Clusters_cluster-info.csv"))

#make final plots
gene_order_pool <- cluster_df_edit_pooled$gene
H_LFC_pooled_reorder <- H_LFC_pooled[gene_order_pool,]
H_TE_pooled_reorder <- H_TE_pooled[gene_order_pool,]

leftAnnot_pool <- cluster_df_edit_pooled

p_LFC_pooled2 <- Heatmap(H_LFC_pooled_reorder,
        column_title = "LFC",
        column_title_gp = gpar(fontsize=12),
        row_title_gp = gpar(fontsize=7),
        name = "log2(treated/control)",
...
p_TE_pooled2 <- Heatmap(H_TE_pooled_reorder,
        column_title = "TE",
        column_title_gp = gpar(fontsize=12),
        name = "log2(((heavy)(medium))/light)",
        show_row_names = F,
        ...
        col = colorRamp2(c(-3,-1.5,-0.5 ,0,0.5, 1.5,3), c("darkblue", "#8C9FCA",
                                                            "white", "white", "white",
                                                           "orangered","firebrick")))
#save even though clusters are wrong
p_LFC_pooled2 + p_TE_pooled2
do_plot("TE_analysis/FDR0.01/FDR0.01_LFC1/ComplexHeatmap/LFC_TE_2-6h_Compiled_8_Clusters.pdf", 8, 6, temp)
...
cpm_df_int_test = cpm_all %>%
              data.frame() %>%
              rownames_to_column(var = "gene") %>%
              filter(gene %in% int_genes[[1]] |
                       gene %in% int_genes[[2]] |
                       gene %in% int_genes[[3]]) %>%
...
#plot
p<-ggplot(cpm_df_int_test %>% filter(gene %in% int_genes[[1]]),
       aes(x = Pool, y = Avg_cpm, color = Treatment)) +
       geom_line(size=1) +
       geom_point() +
       geom_errorbar(aes(ymin=Avg_cpm - se, ymax= Avg_cpm + se), width=0.25, size=1) +
       scale_color_manual(values = c(Control = "grey",
                                     LSG2h = "#FAC5B0",
                                     LSG6h = "#F68064")) +
       facet_wrap(~gene, scales="free_y") #+
       scale_x_discrete(name = "Polysome Pool", limits=factor(c(0,1,2,3)))

save_plot(path(output_dir,
               "TE_analysis/FDR0.01/FDR0.01_LFC1/mRNA_TE_correlation/TE_up_gene-lists/single_gene_plots/LSG2h_genes_int_withINPUT.pdf"),
          p)
