...
do_plot("TE_analysis/FDR0.01/FDR0.01_LFC1/ComplexHeatmap/LFC_TE_2-6hours_clustered-by-2h.pdf", 8, 6, temp2)

#pool LFC and TE heatmaps then replot (assume order is constant)
sum(rownames(H_LFC_reorder) == rownames(H_LFC6)) #check -- all good
sum(rownames(H_TE_reorder) == rownames(H_TE6)) # also good
H_LFC_pooled <- cbind(H_LFC_reorder, H_LFC6)
H_TE_pooled <- cbind(H_TE_reorder, H_TE6)
...
#save plot for now
do_plot("TE_analysis/FDR0.01/FDR0.01_LFC1/ComplexHeatmap/LFC_TE_2-6h_20Clusters.pdf",8,6,LFC_pooled_drawn)
...
cluster_df_edit_pooled = cluster_df_pooled %>%
                  mutate(old_cluster = Cluster, 
                         Cluster = ifelse(old_cluster %in% c(18:20), 1,
                                    ifelse(old_cluster %in% c(17,15), 2,
                                    ifelse(old_cluster %in% c(14,16), 3,
                                    ifelse(old_cluster %in% c(13), 4,
                                    ifelse(old_cluster %in% c(12), 5, 
                                    ifelse(old_cluster %in% c(11,4,8,10), 6, 
                                    ifelse(old_cluster %in% c(9,7,3), 7, 8)))))))) %>%
                  arrange(Cluster)

rownames(cluster_df_edit_pooled) = cluster_df_edit_pooled$gene

write.csv(as.matrix(cluster_df_edit_pooled), path(output_dir, "TE_analysis/FDR0.01/FDR0.01_LFC1/ComplexHeatmap/20231109_pooledLFC_TE_compiledHeatmap_8Clusters_cluster-info.csv"))
...
p_LFC_pooled <- Heatmap(H_LFC_pooled_reorder,
        column_title = "LFC",
        column_title_gp = gpar(fontsize=12),
        row_title_gp = gpar(fontsize=7),
        name = "log2(treated/control)",
...
p_TE_pooled <- Heatmap(H_TE_pooled_reorder,
        column_title = "TE",
        column_title_gp = gpar(fontsize=12),
        name = "log2(((heavy)(medium))/light)",
        show_row_names = F,
