...
p<-ggplot(comparison_df, aes(x = `LFC_Total`, y = `LFC_TE`, color = category)) +
  geom_point(data = subset(comparison_df, category == "no change"), size =0.75) +
  geom_point(data = subset(comparison_df, category != "no change" & !(sig_Total != 0 | sig_TE != 0)),
  ...
  geom_text_repel(data = subset(comparison_df, LFC_TE > 0.5 & LFC_Total <0.5),
                  aes(label = gene),
                  color = "black",
                  size = 2,
                  max.overlaps = 25,
  ...
  labs(y = expression("log"["2"] ~ "FC  TE"),
       x = expression("log"["2"] ~ "FC  Total mRNA")) +
  theme_classic() +
...
save_plot(path(output_dir, 
               "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/correlation_plots/mRNA/20240116_TotalmRNA-v-TE-correlation_genes_labeled3.pdf"), 
         p)
```
