...
H_per = H[rownames(H) %in% rownames(DE_LFC_thresh),]
H_per_scale = H_per %>%
               t() %>%
               scale() %>%
               t()
...
p_LFC <-Heatmap(H_LFC,
        ...
        top_annotation = HeatmapAnnotation(df = topAnnot_LFC,
                         col = list(Treatment = c(Control = "grey",
                          ...
                          show_legend = F,
                          annotation_name_gp = gpar(fontsize = 8)),
        column_split = topAnnot_LFC$`Polysome Fraction`,
        column_gap = unit(2, "mm"),
        row_km = 20,
        row_gap = unit(0, "mm"),
        #col = colorRamp2(c(-3, 0, 3), c("white", "#8C9FCA", "darkblue"))
        )
intermed_p <- p_per + p_LFC

#save intermediate heatmap
do_plot("TE_analysis/FDR0.01/FDR0.01_LFC1/complexHeatmap/percent-transcript_PLUS_LFC_2883genes.pdf",
        8,6,intermed_p)

#make TE heatmap
H_TE = DE_LFC_thresh %>%
        select(contains("TE"), contains("mRNA")) %>%
        as.matrix()
#reorder data to match
H_TE = H_TE[rownames(H_per),]
...
p_TE <- Heatmap(H_TE,
        ...
        top_annotation = HeatmapAnnotation(df = topAnnot_TE,
                         col = list(Treatment = c(Control = "grey",
                                                  ...
                                                  LSG6h = "#F68064"),
                                   Metric = c("TE" = "pink", 
                                              "delta.mRNA" ="lightyellow")),
                          border = T,
                          show_legend = F,
                          annotation_name_gp = gpar(fontsize = 8)),
        column_split = topAnnot_TE$Metric,
        column_gap = unit(2, "mm"),
        row_km = 20,
        row_gap = unit(0, "mm"),
        col = colorRamp2(c(-3, 0, 3), c("green", "white", "pink"))
        )

#put it all together
first_try <-p_per + p_LFC +p_TE

#first iteration plot save
do_plot("TE_analysis/FDR0.01/FDR0.01_LFC1/compiled_heatmap_test.pdf", 8,6,first_try)
