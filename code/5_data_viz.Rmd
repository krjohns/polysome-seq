...
# For saving plots  
output_dir = fs::path(wd, "../output/2023_10_30-LSG")
...
#write.csv(as.matrix(cluster_df_edit), 
         # path(output_dir,
     #            "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/20240108_2h6h_compiled_lfc_heatmap_fdr0.1_lfc0.5_8-clust#ers.csv"))
...
```{r}
interesting_genes = read_csv(path(output_dir,
    "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/complex_heatmap/cluster_lists/20240115_TE_genes-of-interest_by-cluster_short.csv"))
                                   
#prep cpm data for plotting
cpm_df_cluster = cpm_all %>%
              data.frame() %>%
              rownames_to_column(var = "gene") %>%
              filter(gene %in% interesting_genes$gene %>% unlist()) %>%
              ...
              separate(Treatment_Rep_Pool, 
                       ...
                       sep = "_") %>%
              #drop polysome pools (calculated from counts at the beginning)
              filter(!(grepl("Polysome", Pool))) %>%
              pivot_wider(names_from = "Rep", values_from = "cpm") %>%
              #calculate average and se
              mutate(
              Pool = ifelse(Pool == "Input", 0, 
                as.numeric(gsub("Pool", "", Pool))),
              Avg_cpm = ifelse(is.na(Rep1), (Rep2 + Rep3)/2, (Rep1 + Rep2 + Rep3)/3),
              se = ifelse(is.na(Rep1),  
                sqrt(((Rep2-Avg_cpm)^2+(Rep3-Avg_cpm)^2)/2)/sqrt(2),
                sqrt(((Rep1-Avg_cpm)^2+(Rep2-Avg_cpm)^2+(Rep3-Avg_cpm)^2)/3)/sqrt(3))
              ) %>%
             full_join(interesting_genes, by = "gene")
#plot
p_list <- list() #initialize empty list
for(i in c(1:7)){
p_list[[i]]<-ggplot(cpm_df_cluster %>% 
       filter(gene %in% (interesting_genes %>% 
                                filter(Cluster == i) %>%
                                select(gene) %>%
                                unlist())),
       aes(x = Treatment, y = Avg_cpm, fill = str_c(Treatment, "_", as.character(Pool)), 
           alpha = as.character(Pool))) +
       geom_bar(position = "dodge", width =0.75, stat = "identity", color = "black") +
       #geom_point() +
       geom_errorbar(position = position_dodge(width=0.75), 
                     aes(x = Treatment, ymin=Avg_cpm - se, ymax= Avg_cpm + se),
                     width=0.5, size=0.5, color = "black", alpha = 1) +
       scale_fill_manual(values = c("grey", "grey","grey","grey",
                                   "orange","#FAC5B0","#FAC5B0","#FAC5B0",
                                   "#C46650", "#F68064", "#F68064","#F68064")) +
       scale_alpha_manual(values= c(0.1, 0.3, 0.7, 1)) +
       facet_wrap(~gene, scales="free", nrow =1) +
       #scale_y_continuous(labels = function(x) format(x, scientific = TRUE)) +
       labs(y = str_c("Cluster ", as.character(i))) +
       #labs(x = ifelse(i==7, "Polysome Pool", element_blank())) +
       theme(text = element_text(size=rel(5)),
             ...
             plot.margin = unit(c(0.001, 0.00001, 0.001, 0.00001), "inches"))

#save_plot(path(output_dir,
         #      str_c("TE_analysis/2024_01_TE/FDR0.1/LFC0.5/single_gene_plots/heatmap_clusters/Cluster_",
         #            as.character(i), "_grouped-by-treat.pdf")),
         # p_list[[i]])
}


#add x axis text to bottom plot
p_list[[7]] = p_list[[7]] +
         theme(axis.text.x = element_text(size = rel(1.65)))

p_list_up = p_list[1:4]
p_list_down = p_list[5:7]
p_list_up[[4]] = p_list[[4]] +
         theme(axis.text.x = element_text(size = rel(1.65)))

p_grid <- ggarrange(plotlist = p_list_down, ncol=1, align = "hv") 
#p_grid <- annotate_figure(p_grid, 
                         # top = "Average cpm",
                         #bottom = "Polysome Pool")

p_grid 


save_plot(path(output_dir,
               "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/single_gene_plots/heatmap_clusters/compiled_cluster_grid_grouped-by-treat-DOWN.pdf"),
          p_grid,
          base_aspect_ratio=1.5)


#save df
write.csv(as.matrix(cpm_df_cluster), path(output_dir,
               "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/complex_heatmap/cluster_lists/20240115_single_gene_plot_cpm_values-by_cluster.csv"))


```

#single gene plots of non-translational machinery TE up
```{r}
cluster2.3_genes <-read_csv(fs::path(output_dir,
    "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/complex_heatmap/cluster_lists/20240115_final_7_clusters.csv")) %>%
      dplyr::select(gene, Cluster) %>%
      filter(Cluster %in% c(2,3)) %>%
      filter(!grepl("Rpl", gene)) %>%
      filter(!grepl("Rps", gene)) %>%
      filter(!grepl("Eif", gene)) %>%
      filter(!grepl("Eef", gene)) %>%
      filter(!grepl("\\.", gene)) %>%
      arrange(desc(gene)) #required for clusterProfiler gseGO fucntion

gene_list = DE_LFC_thresh %>%
            dplyr::select(starts_with("TE")) %>%
            rownames_to_column(var = "gene") %>%
            filter(gene %in% cluster2.3_genes$gene) #%>%
            column_to_rownames(var = "gene")

#gene_list = sort(gene_list, decreasing =T)

#from cluster Profilier
gse <- gseGO(geneList=gene_list$gene, 
             ont ="BP", 
             keyType = "SYMBOL", 
             nPerm = 10000, 
             minGSSize = 3, 
...
groupGO.df = groupGO.df %>%
             mutate(Description= factor(Description,
                          levels = unique(groupGO.df$Description)))

heatmapGO = groupGO.df #%>%
            


p<-heatplot(groupGO, showCategory = 20) +
    theme(text = element_text(size = rel(3)))
save_plot(fs::path(output_dir, "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/complex_heatmap/clusterProfiler/groupGO_BP_heatmap.pdf"),p)

#trying my own heatmap
#need a more automated way to do this...
df.1 <- groupGO.df %>% 
      dplyr::select(Description, geneID) %>%
      mutate(Alpk2 = ifelse(grepl("Alpk2", geneID), 1, 0),
             Btf3 = ifelse(grepl("Btf3", geneID), 1, 0),
             Cand2 = ifelse(grepl("Cand2", geneID), 1, 0),
             Cct4 = ifelse(grepl("Cct4", geneID), 1, 0),
             Cct5 = ifelse(grepl("Cct5", geneID), 1, 0),
             Chp1 = ifelse(grepl("Chp1", geneID), 1, 0),
             Crbn = ifelse(grepl("Crbn", geneID), 1, 0),
             Fau = ifelse(grepl("Fau", geneID), 1, 0),
             Gm2000 = ifelse(grepl("Gm2000", geneID), 1, 0),
             Gm3550 = ifelse(grepl("Gm3550", geneID), 1, 0),
             Hspa8 = ifelse(grepl("Hspa8", geneID), 1, 0),
             
             Igbp1 = ifelse(grepl("Igbp1", geneID), 1, 0),
             Inpp5e = ifelse(grepl("Inpp5e", geneID), 1, 0),
             Kxd1 = ifelse(grepl("Kxd1", geneID), 1, 0),
             Lamc1 = ifelse(grepl("Lamc1", geneID), 1, 0),
             Mapk6 = ifelse(grepl("Mapk6", geneID), 1, 0),
             Med21 = ifelse(grepl("Med21", geneID), 1, 0),
             Memo1 = ifelse(grepl("Memo1", geneID), 1, 0),
             Naca = ifelse(grepl("Naca", geneID), 1, 0),
             Nfkbid = ifelse(grepl("Nfkbid", geneID), 1, 0),
             Pabpc1 = ifelse(grepl("Pabpc1", geneID), 1, 0),
             Pfdn5 = ifelse(grepl("Pfdn5", geneID), 1, 0),
             
             Polr1c = ifelse(grepl("Polr1c", geneID), 1, 0),
             Prex2 = ifelse(grepl("Prex2", geneID), 1, 0),
             Psmd13 = ifelse(grepl("Psmd13", geneID), 1, 0),
             Psmd4 = ifelse(grepl("Psmd4", geneID), 1, 0),
             Rack1 = ifelse(grepl("Rack1", geneID), 1, 0),
             Ralgapa1 = ifelse(grepl("Ralgapa1", geneID), 1, 0),
             Rbmx = ifelse(grepl("Rbmx", geneID), 1, 0),
             Serpinb6b = ifelse(grepl("Serpinb6b", geneID), 1, 0),
             Snrpb2 = ifelse(grepl("Snrpb2", geneID), 1, 0),
             Srsf2 = ifelse(grepl("Srsf2", geneID), 1, 0),
             Tctn1 = ifelse(grepl("Tctn1", geneID), 1, 0),
             
             Tlcd1 = ifelse(grepl("Tlcd1", geneID), 1, 0),
             Tpt1 = ifelse(grepl("Tpt1", geneID), 1, 0),
             Uba52 = ifelse(grepl("Uba52", geneID), 1, 0),
             Uqcrfs1 = ifelse(grepl("Uqcrfs1", geneID), 1, 0),
             Srsf2 = ifelse(grepl("Srsf2", geneID), 1, 0),
             Wdr38 = ifelse(grepl("Wdr38", geneID), 1, 0),
             
             Ybx1 = ifelse(grepl("Ybx1", geneID), 1, 0),
             Zfp263 = ifelse(grepl("Zfp263", geneID), 1, 0),
             Zfp931 = ifelse(grepl("Zfp931", geneID), 1, 0),
             Zrsr1 = ifelse(grepl("Zrsr1", geneID), 1, 0)
             ) 

sums = data.frame(gene = colnames(df.1 %>% dplyr::select(Alpk2:Zrsr1)),
                  sum = colSums(df.1 %>% dplyr::select(Alpk2:Zrsr1)))
   
df.2 = df.1 %>%
    pivot_longer(cols = -c(Description, geneID),
                 names_to = "gene", values_to = "is-part") %>%
    left_join(gene_list, by = "gene") %>%
    left_join(sums, by = "gene") %>%
    arrange(desc(sum)) 

df = df.2 %>%
     mutate(gene = factor(gene, levels = unique(gene)))

ggplot(df, aes(x = gene, y = Description, fill = `is-part`*TE_LSG2h)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "red") +
  theme(axis.text.x = element_text(angle = 45, size = rel(3), hjust = 1),
        legend.position = "bottom")

save_plot(fs::path(output_dir, "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/complex_heatmap/clusterProfiler/groupGO_kates_heatmap.pdf"), p, base_aspect_ratio = 3)

p<-ggplot(groupGO.df, aes(y = Description, x = Count)) +
  geom_col() +
  xlim(0,50) +
  geom_text(mapping = aes(x = Count + 3, label = GeneRatio), size = 2) #+
  #geom_text(mapping = aes(x = Count + 8, label = geneID), color = "red",
         #   size = 2)
save_plot(fs::path(output_dir,"TE_analysis/2024_01_TE/cluster2-3_groupGO_barchart.pdf"),p)
...
write.csv(as.matrix(GO.df), fs::path(output_dir, "TE_analysis/2024_01_TE/cluster2-3_enrichGO_BP.csv"))
p<-dotplot(GO) +theme(axis.text.y = element_text(size = 6))
save_plot(fs::path(output_dir, "TE_analysis/2024_01_TE/cluster2-3_enrichGO_BP.pdf"),p)
p
```


#mRNA v TE correlation plot
preliminary plot (TE v. Total mRNA)
```{r}
#start by trimming DE binary significance dataframe to necessary comparisons
...
comparison_df = log2FC.all %>%
                #select(contains("Total"), starts_with("TE")) %>%
                ...
                #mutate(metric = str_c("LFC (", metric, ")")) %>%
                ...
                mutate(category = ifelse(LFC_TE > 0.5 & (LFC_Total < 0.5 & LFC_Total > -0.5),
                                         "TE UP (no \U0394 mRNA)",
                                         ifelse(LFC_TE < -0.5 & (LFC_Total < 0.5 & LFC_Total > -0.5),
                                         "TE DOWN (no \U0394 mRNA)",
                                         ifelse(LFC_TE > 0.5 & LFC_Total > 0.5,
                                         "mRNA UP and TE UP",
                                         ifelse(LFC_TE < -0.5 & LFC_Total < -0.5,
                                         ...
                                         ifelse(LFC_TE > 0.5  & LFC_Total < -0.5,
                                         "mRNA DOWN and TE UP",
                                         ifelse((LFC_TE < 0.5 & LFC_TE > -0.5) & LFC_Total > 0.5,
                                         "mRNA UP (no \U0394 TE)",
                                         ifelse((LFC_TE < 0.5 & LFC_TE > -0.5) & LFC_Total < -0.5,
...
comparison_data_pivot = comparison_df %>%
                        pivot_longer(cols = c(LFC_L:LFC_H), names_to = "Pool", values_to = "LFC_Pool") %>%
                        mutate(Pool = gsub("LFC_L", "Pool 1 (1 rib.)", 
                                           gsub("LFC_M", "Pool 2 (2-3 rib.)", 
                                                gsub("LFC_H", "Pool 3 (4+ rib.)", Pool))))

p<-ggplot(comparison_data_pivot, aes(x = `LFC_Total`, y = `LFC_Pool`, color = category)) +
  geom_point(data = subset(comparison_data_pivot, category == "no change"), size =0.75) +
  geom_point(data = subset(comparison_data_pivot, category != "no change" & !(sig_Total != 0 | sig_TE != 0)),
             size =0.75, color = "#BFBEBE") +
  geom_point(data = subset(comparison_data_pivot, category != "no change" & (sig_Total != 0 | sig_TE != 0)), 
             size =0.75)  +
  ...
  #geom_text_repel(data = subset(comparison_data_pivot, LFC_TE > 0.5 & LFC_Total <0.5),
              #    aes(label = gene),
               #   color = "black",
               #   size = 2,
               #   max.overlaps = 25,
               ...
               #   #force_pull=10,
                ...
                #  seed=123) +
  facet_wrap(~Pool) +
  labs(y = expression("log"["2"] ~ "FC  Ribosomal mRNA"),
       x = expression("log"["2"] ~ "FC  Total mRNA (Input)")) +
  theme_classic() +
  theme(strip.background = element_rect(color = "white"),
        ...
        plot.margin = unit(c(0.001,0.1,0.001,0.1), "inches"))


#save_plot(path(output_dir, 
            #   "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/correlation_plots/mRNA/20240116_TotalmRNA-v-Pool.pdf"), 
        # p)
```


save interesting / relevant gene sets
```{r}
LSG2h <- comparison_df %>% 
...
write.csv(as.matrix(LSG2h_only),
          path(output_dir,
               "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/correlation_plots/mRNA/TE_up_mRNA_noChange_genes_LFC0.5_sigFDR0.1_LSG2h_UNIQUE.csv"))
write.csv(as.matrix(LSG2h),
          path(output_dir, 
               "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/correlation_plots/mRNA/TE_up_mRNA_noChange_genes_LFC0.5_sigFDR0.1_LSG2h.csv"))
write.csv(as.matrix(overlap),
          path(output_dir, 
               "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/correlation_plots/mRNA/TE_up_mRNA_noChange_genes_LFC0.5_sigFDR0.1_overlap.csv"))

#get group of genes that are interesting for follow-up
int_genes <- subset(comparison_df, abs(`LFC_Total`) < 0.5 & abs(`LFC_TE`) > 0.5)
int_genes_up <- subset(comparison_df, abs(`LFC_Total`) < 0.5 & `LFC_TE` > 0.5)
...
#write.csv(as.matrix(int_genes_sig), path(output_dir, #"TE_analysis/2024_01_TE/FDR0.1/LFC0.5/correlation_plots/mRNA/TE_up_mRNA_noChange_genes_LFC0.5_sigFDR0.1.csv"))
```
...
prot_TE_only_df = prot_TE_mRNA_merge %>% 
                  data.frame() %>%
                  dplyr::select(gene, Protein_time, Protein_LFC, Protein_category, LFC_H, TE_time) %>%
                  unique() %>%
                  mutate(Protein_category = gsub(".sig", "", Protein_category))
  


p<-ggplot(prot_TE_only_df, aes(x = Protein_LFC,
                                y = LFC_H, 
                                color = ifelse(`Protein_category` == "additive", "F", "T"))) +
  geom_point(alpha = 1, size = 0.1) +
  geom_point(data = subset(prot_TE_only_df, Protein_category != "additive"),
             alpha = 1, size = 0.1) +
  facet_grid(TE_time~Protein_time) +
  scale_color_manual(values = c("T" ="black", "F" = "grey75")) +
  geom_text_repel(data = subset(prot_TE_only_df,
                                `Protein_category` == "synergy"),
                  aes(label = gene),
                  color = "red",
                  size = 1,
                  max.overlaps = 10,
  ...
  labs(x = "Protein (LFC)", y = "Heavy Polysomes (LFC)") +
...
save_plot(fs::path(output_dir, "TE_analysis/2024_01_TE/FDR0.1/LFC0.5/correlation_plots/Protein/Protein_heavy_polysome_correlation_non-additive-genes.pdf"),p, dpi =90)
```
